...
//    $result = $this->db->executeQuery($sql);
    ...
    on r." . $this->config->COL_userRegistration_unique_id . " = u." . $this->config->COL_users_userRegistration_unique_id . "
        ...
        $this->sendMail($this->getEmail(), "Reset Password Request - CrypBrokers", dbconfig::emailContentResetPassword($name, "http://localhost/kyc/resetPassword.html?token=" . $token));
    ...
    public function addEmailVerificationToken()
    {
        ...
        $this->sendMail($this->getEmail(), "Email Verification - wisepass", dbconfig::emailContentEmailVerification("http://localhost/kyc/users/verifyEmail.php?token=" . $token));

        $response['success'] = true;
        $response['result'] = "An email has been sent to your registered email.";
        return $response;
    }
